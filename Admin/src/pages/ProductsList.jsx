...
const perfumeSizeOptions = ["30ML", "50ML", "75ML", "100ML", "3 x 10ML"];
...
const ProductsList = ({ token }) => {
  ...
  const stats = useMemo(() => {
    ...
    const bestseller = items.filter((item) => item.bestseller).length;
    ...
    <main className="mx-auto w-full max-w-[1480px] text-[#1f1b17]">
      ...
      {items.length === 0 ? (
        <section className="rounded-md border border-[#eadfd2] bg-[#fffaf4] p-10 text-center shadow-[0_18px_45px_rgba(62,45,28,0.08)]">
          <p className="font-serif text-3xl text-[#1f1b17]">No products yet</p>
          <p className="mt-3 text-sm text-[#7d6756]">
            Add the first Levon perfume product to start building the catalog.
          </p>
          <Link
            to="/add"
            className="mt-6 inline-flex rounded-full bg-[#1f1b17] px-6 py-3 text-xs font-semibold uppercase tracking-[0.14em] text-white transition hover:bg-[#c49a5e]"
          >
            Add Product
          </Link>
        </section>
        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {items.map((product) => {
            ...
            return (
              <article
                key={product._id}
                className="overflow-hidden rounded-md border border-[#eadfd2] bg-[#fffaf4] shadow-[0_18px_45px_rgba(62,45,28,0.08)]"
              >
                <div className="relative aspect-[4/3] bg-[#eee4d9]">
                  {thumb ? (
                    <img
                      src={thumb}
                      alt={product.name}
                    ...
                    <div className="grid h-full w-full place-items-center text-xs font-semibold uppercase tracking-[0.16em] text-[#a49181]">
                      No Image
                    </div>
                  )}
                  ...
                  <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">
                    ...
                    {product.active === false && (
                      <span className="rounded-full bg-[#1f1b17]/85 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-white shadow-sm">
                        Hidden
                      </span>
                    )}
                  </div>
                  ...
                  {(product.newArrival || product.bestseller || product.onSales) && (
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <h2 className="truncate font-serif text-2xl leading-tight text-[#1f1b17]">
                        {product.name}
                      </h2>
                      <p className="mt-1 truncate text-[11px] uppercase tracking-[0.15em] text-[#8a7b6b]">
                        {[product.category, product.concentration].filter(Boolean).join(" / ")}
                      </p>
                    </div>
                    <span className="shrink-0 rounded-full border border-[#dfd1c1] bg-[#fffdf9] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#7d6756]">
                      {images.length} img
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-baseline gap-2">
                      <span className={`text-base font-semibold ${discount ? "text-[#7b2d2d]" : "text-[#1f1b17]"}`}>
                  ...
                  {sizes.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {sizes.map((size) => (
                        <span
                          key={size}
                          className="rounded-full border border-[#dfd1c1] bg-[#fffdf9] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-[#746657]"
                        >
                          {size}
                        </span>
                      ))}
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-2">
                    <Link
                      to={`/edit/${product._id}`}
                      className="rounded-full bg-[#1f1b17] px-4 py-2 text-center text-xs font-semibold uppercase tracking-[0.12em] text-white transition hover:bg-[#c49a5e]"
                    >
                      Edit
                    </Link>
                    <button
