...
const OnSaleProducts = () => {
  ...
  const saleProducts = useMemo(
    () =>
      (products || []).filter(
        (product) => product?.active !== false && product?.onSales
      ),
    [products]
  );
  ...
  }, [saleProducts, sortType, specialFlags, stockFlags]);
  ...
  const countItems = (predicate) => saleProducts.filter(predicate).length;
    ...
    <div className="divide-y divide-black/15">
      <section className="py-5">
        ...
        <label className="relative block">
          <span className="sr-only">Sort sale products</span>
          <select
            value={selectedSort}
        ...
        <div>
          {productsLoading ? (
            ...
            <section className="mx-auto max-w-xl py-20 text-center">
              <h2 className="text-3xl">No sale products</h2>
              <p className="mt-4 text-sm leading-6 text-black/55">
                There are no sale products matching these filters right now.
              </p>
