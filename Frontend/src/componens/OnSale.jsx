...
const OnSale = () => {
  ...
  const onSales = useMemo(() => products.filter((p) => p.onSales), [products]);
  ...
  if (productsLoading) return <ProductRailSkeleton titleWidth="w-36" />;
  if (!onSales.length) return null;
    ...
    <section className="my-0 py-5 sm:py-6">
      <div className="mb-9 text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[#8a7462] sm:text-base">
          Refined Levon signatures, available for a limited moment.
        </p>
      </div>
                  ...
                  >
                    <div className="relative aspect-[4/5] overflow-hidden rounded bg-[#eadfd2]">
                      ...
                      {hasDiscount ? (
                        ...
                        <span className="absolute left-4 top-4 rounded-full bg-[#c49a5e] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-white">
                          SALE
                        </span>
                      )}
